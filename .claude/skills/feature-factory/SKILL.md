...
- **Every agent ends with a `[factory:<agent>]` comment on the ticket** in the standard format (see `LINEAR-INTEGRATION.md` §2/§4). The agent **returns the comment body**; **the orchestrator posts it as the factory's Linear agent** via `.claude/scripts/factory-linear-comment.sh --issue <OGE-ID> --body <markdown>` (`LINEAR_AGENT_TOKEN`) — **never `linear.save_comment`** (authors as the human operator). Same for the §5 final telemetry comment.
