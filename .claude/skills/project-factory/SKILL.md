...
- **All `[factory:project-factory]` comments above are posted as the factory's Linear agent** via `.claude/scripts/factory-linear-comment.sh` (→ Mission Control `factory-comment`, box→MC bearer; no Linear token on the box), never `linear.save_comment`. See `LINEAR-INTEGRATION.md` §14.
