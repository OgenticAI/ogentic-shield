...
- **All `[factory:project-factory]` comments above are posted as the factory's Linear agent** via `.claude/scripts/factory-linear-comment.sh` (`LINEAR_AGENT_TOKEN`), never `linear.save_comment`. See `LINEAR-INTEGRATION.md` §14.
