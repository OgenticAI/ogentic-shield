...
- If Critical: `linear.save_issue(<TICKET-ID>, addLabels=["security-blocked"])` and, for each deferred Critical, `linear.save_issue(project=<same>, parentId=<TICKET-ID>, title=<short>, description=<detail with file:line and attack scenario>, assignee=<operator/David — never null; §2a>, labels=["from-security"])`.
